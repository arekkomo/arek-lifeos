...
- `/AGENTS/Director/Learnings-Film-Pipeline.md` — accumulated feedback on film sessions (apply all rules)
- `/AGENTS/Director/Learnings-Song-Pipeline.md` — accumulated feedback on song sessions (apply all rules)
...
**Feedback step:** At the end of every film or song session, ask once:
> "Anything to improve on how we ran that session? (Y to give feedback / skip)"
If Arek gives feedback: extract the rule, append to the relevant Learnings file with today's date.
