#!/usr/bin/env python3
"""One-way, idempotent Notion → Obsidian mirror for accessible creative databases.

Writes Notion material into CREATIVE/Notion-Creative-Mirror without touching curated
CREATIVE/Library captures or project folders. Run manually after changing Notion.
"""
...
def main() -> None:
    ...
    for domain, database_id in DATABASES.items():
        try:
            ...
            files = [str(write_page(domain, page).relative_to(ROOT)) for page in pages]
        ...
        time.sleep(0.35)
    (ROOT / 'SYNC-REPORT.json').write_text(json.dumps(report, indent=2, ensure_ascii=False) + '\n')
    ...
    print(f'Synced {total} Notion records across {len(DATABASES) - len(unavailable)} creative databases to {ROOT}{suffix}')
