...
- Portrait makeup: dozens of styles from beauty to Halloween
- Text/typography style transfer (comparable to closed-source)
- Photo restoration: old photo repair and enhancement
- ComfyUI node support + GGUF lightweight format
- Full LoRA training code (HSDP/FSDP)
...
- Portrait makeup application for look development
- Archival reference footage and production still restoration
- Typography / title design via text style transfer
- Multi-image composite direction for concept art (no 3D needed)
- ComfyUI instruction-based editing node integration

## Requirements

- 30GB VRAM minimum for optimized inference
- Optional: Gemini API key or MiniMax for Agent recaption
...
v1.1 improves portrait consistency, multi-element fusion, text reference, and makeup. Training: Pretrain → SFT → RL, backbone-agnostic. REDEdit-Bench is their evaluation benchmark.