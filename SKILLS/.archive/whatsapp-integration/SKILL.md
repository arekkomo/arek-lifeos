...
```yaml
whatsapp:
  bridge_path: /home/realityrove/.hermes/hermes-agent/scripts/whatsapp-bridge/bridge.js
...
```
✗ Bridge script not found at /home/realityrove/.local/lib/python3.12/site-packages/scripts/whatsapp-bridge/bridge.js
```
...
ln -s /home/realityrove/.hermes/hermes-agent/scripts/whatsapp-bridge/ /home/realityrove/.local/lib/python3.12/site-packages/scripts/whatsapp-bridge
```
...
**Fix:** `fuser 3000/tcp` to find the occupant. Then either stop the occupant or change `bridge_port` in config.yaml. See `references/eaddrinuse-diagnostics.md` for the full diagnostic sequence (pre-flight port check, config-vs-reality verification).
...
- **EADDRINUSE and port occupancy** has a dedicated guide in `references/eaddrinuse-diagnostics.md`
