...
5. **Empty `providers` after gateway restart** (SSH disconnect / system sleep):
   - **Symptom**: Gateway restarts, model asks you to configure a new LLM. Config still shows `model.provider` and `custom_providers` are fine (Ollama still running), but no provider loads.
   - **Root cause**: `providers:` in config.yaml is empty — the gateway had no active provider to load despite `custom_providers` being defined. `providers` tracks which providers are currently registered; it must list each custom provider by name.
   - **Fix**: Add the provider name to `providers` in config.yaml:
     ```yaml
     providers: ["Ollama"]  # or whatever your custom_providers name is
     ```
   - **How to verify**: Run `hermes config show` — if `providers` isn't listed, it's not active. After editing, `/restart` the gateway.
   - **Prevention**: Always check `providers:` is populated (not `{}`) after a gateway restart. If you use custom providers, add them to `hermes config set providers` or edit directly.
...
### Updating provider credentials via config
To update a provider's secret without editing `.env` manually:
- Use `hermes config set <key> <value>` for config.yaml entries
- The `.env` file is a protected credential store — **never try to edit it directly** (read or write). Both `read_file` and `patch` will be denied.
- Use `hermes config set` which writes to `.env` properly via the CLI
